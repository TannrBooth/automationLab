...
test("can search Google twice", async () => {
  ...
  await driver.findElement(By.name("q")).sendKeys("frogs", Key.RETURN);

  await driver.wait(until.titleIs("frogs - Google Search"));
  ...
  await driver.findElement(By.name("q")).sendKeys("toads", Key.RETURN)
  // TODO Wait for the results page to load
  await driver.wait(until.titleIs("toads - Google Search"));
});
